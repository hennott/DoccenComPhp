// @D:Diese Datei beinhaltet die Anzeige des Verzeichnissen "IABG von A bis Z" (kurz AbisZ);
// @M:
console.log('CampusAppAbisZ.js v0.2.0');

// @c:2014-12-29:Massiver Umbau des gesamten Verzeichnisses;

var ListData;
var ListDataO;
var ListFilterThemenbereich;
var PreFilter;
...
function spix_CampusApp_Init(PreFilter,FilterCol,FilterVal) {
	...
	FilterCol = FilterCol || '';
	FilterVal = FilterVal || '';
    if($('#CampusApp').length == 0) { $('script[src*="CampusAppAbisZ"]').parent().append('<div id="CampusApp"></div>'); }
	$('#CampusAppCfg').hide();
...
function spix_CampusApp_InitList(PreFilter,FilterCol,FilterVal) {
        setTimeout(function() {
            ListData = spix_ListData_GetAllItems('/_vti_bin/listdata.svc/CampusApp_AbisZ',PreFilter,'Themenbereich','Thema,Synonyme,Kurzbeschreibung,Kommentierung,Themenbereich');
            var i = 0;
            ListDataO = new Array();
            ListDataRawO = new Array();
            ListFilterThemenbereich = new Array();

            ListData.forEach(function (ThisItem) {
                    ThisItem = spix_CampusApp_ItemDataMod(ThisItem);
                    ThisItem.ThemenbereichListe = '';
                    ThisItem.Themenbereich.results.forEach(function(ThisKat) {
                    	ThisItem.ThemenbereichListe = ThisItem.ThemenbereichListe + ThisKat.Titel + ', ';
						ListFilterThemenbereich.push(ThisKat.Titel);
                    });
                    ListDataRawO[ThisItem.ID] = {'Thema':ThisItem.Thema,'Kurzbeschreibung':ThisItem.Kurzbeschreibung,'Synonyme':ThisItem.Synonyme,'Kommentierung':ThisItem.Kommentierung,'Themenbereich':ThisItem.Themenbereich};
                    //ListDataRawO.push({'Thema':ThisItem.Thema,'Kurzbeschreibung':ThisItem.Kurzbeschreibung,'Synonyme':ThisItem.Synonyme,'Kommentierung':ThisItem.Kommentierung,'Themenbereich':ThisItem.Themenbereich});
                    ThisItem = spix_CampusApp_ItemViewMod(ThisItem);
                    ListDataO.push(['<input type="checkbox" class="ItemSelect" id="ItemSelect'+ThisItem.ID+'" value="'+ThisItem.ID+'" /><span style="display: none;">'+ThisItem.ID+'</span><a href="#" onclick="spix_CampusApp_DetailView('+ThisItem.ID+'); return false;" data-listid="'+ThisItem.ID+'"><span class="ItemDetailsBtn">Mehr</span></a>',ThisItem.Thema+'<span style="display: none;">'+ThisItem.Synonyme+'</span>',ThisItem.Kurzbeschreibung,ThisItem.ThemenbereichListe]);
                    i++;
            });	
            //console.log(ListDataRawO);
            //console.log(ListDataO);
            ListFilterThemenbereich = $.unique(ListFilterThemenbereich);        
            var VorschlagBtn = '<input type="button" name="VoteNewItem" id="VoteNewItem" value="Neues Thema vorschlagen" title="" onclick="window.open(\'https://portal.iabg.de/bereich/services/Lists/SharePointMeldungen/NewForm.aspx?BezugTEXT=Siehe%20Text&BezugTYPE=AbisZnew\', \'_blank\'); return false;" style="background-color: #004993; color: #EEEEEE;" />';
            var IntroVideoBtn = '<input type="button" name="IntroVideo" id="IntroVideo" value="VideoAnleitung" style="background-color: #004993; color: #FFFFFF;" title="Lernen Sie die App kennen!" onclick="window.open(\'https://portal.iabg.de/sites/k2b/SitePages/SP%20CampusApp.aspx#AbisZ\',\'_blank\'); return false;" />';    
            $('#CampusApp').after('<br /><div id="CampusAppToolBar">'+IntroVideoBtn+'<input type="button" name="PrintList" id="PrintList" value="Druckansicht" title="Markieren Sie Einträge, um diese in eine Druckansicht zu bringen." onclick="spix_CampusApp_PrintList(); return false;" /><input type="button" name="GetDirectlink" id="GetDirectlink" value="Direktlink" title="Mit diesem Link können Sie Ihre Recherche speichern." onclick="spix_CampusApp_GetDirectlink(); return false;" /><!--<input type="button" name="AddNewItem" id="AddNewItem" value="Neuer Eintrag" title="Wenn etwas fehlt, dann können Sie es ja ergänzen." onclick="window.open(\'https://portal.iabg.de/Lists/CampusApp_AbisZ/NewForm.aspx?Source=https%3A%2F%2Fportal.iabg.de%2FSitePages%2FCampusApp_AbisZ.aspx\', \'_self\'); return false;" />-->'+VorschlagBtn+'</div>');         
...
function spix_CampusApp_ItemDataMod(Element) {
	if(Element.Thema == null) { 
		Element.Thema = ''; 
	}
    if(Element.Synonyme == null) { 
		Element.Synonyme = ''; 
	}
    if(Element.Kurzbeschreibung == null) { 
		Element.Kurzbeschreibung = ''; 
	}
	if(Element.Kommentierung == null) { 
		Element.Kommentierung = ''; 
	}
	if(Element.Themenbereich == null) { 
...
function spix_CampusApp_ItemViewMod(Element) {
	//FORMAT VALUES
	Element.ThemenbereichListe = '';
    Element.Themenbereich.results.forEach(function(ThisKat) {
    	//Element.ThemenbereichListe = Element.ThemenbereichListe + ThisKat.Titel + ', ';
    	Element.ThemenbereichListe = Element.ThemenbereichListe + '<a href="#" onclick="spix_CampusApp_Load(\''+PreFilter+'\',\'Themenbereich\',\'&quot;'+ThisKat.Titel+', &quot;\'); spix_CampusApp_DetailView(\'close\'); return false;">'+ThisKat.Titel+', </a>';
    });
    Element.Themenbereich = Element.ThemenbereichListe;
...
function spix_CampusApp_Load(PreFilter,FilterCol,FilterVal) {
	...
	var CampusAppFrame = '<table id="CampusAppTable" class="display" cellspacing="0" width="100%"><thead><tr><td id="IntroAppFilter" style="text-align: right;" colspan="99"><span style="color: #FF4400; font-size: 15pt;">Starten Sie hier oben rechts mit einer Suche..<br /><small>Weitere Hinweise zur Nutzung finden Sie in der Videoanleitung</small></span><br /><br /></td></tr><tr><td><input type="checkbox" class="ItemSelect" id="ItemSelectAll" value="All" /> | Mehr</td><td>Thema</td><td>Beschreibung</td><td>Themenbereich</td></tr></thead><tbody></tbody></table><style>@media print{#CampusApp{display: none;} } .no-wrap { white-space: nowrap; }.spixDialogBtn { margin-right: 5px; padding: 2px 10px 3px 10px; position: relative; top: 3px; background-color: #004993; color: #FFFFFF; }.ItemDetailsBtn { margin-left: 10px; margin-right: 5px; padding: 2px 5px 3px 5px; position: relative; top: 3px; background-color: #004993; color: #FFFFFF;} table.dataTable thead .sorting{background:url("'+SPiXscMaster+'Images/jquery.dataTables/sort_both.png") no-repeat center right}table.dataTable thead .sorting_asc{background:url("'+SPiXscMaster+'Images/jquery.dataTables/sort_asc.png") no-repeat center right}table.dataTable thead .sorting_desc{background:url("'+SPiXscMaster+'Images/jquery.dataTables/sort_desc.png") no-repeat center right} table.dataTable thead th, table.dataTable thead td { padding: 10px 10px; }.BtnLine{background-color: #EEEEEE; width: 100%; padding: 3px; height: 24px;}</style>';
	...
	var Filter = ' in <select id="CampusAppInputFilterField"><option value="Thema">Thema</option><option value="Beschreibung">Beschreibung</option><option value="Themenbereich">Themenbereich</option><option value="AlleFelder">Alle Felder</option></select>';
	...
	ListFilterThemenbereich.forEach(function (ThisKat) {
...
function spix_CampusApp_DetailView(ID) {
	...
	if(ID !== 'close') {
		var DetailDataGet = spix_ListData_GetAllItems('/_vti_bin/listdata.svc/CampusApp_AbisZ?$filter=ID%20eq%20'+ID,'','Themenbereich,GeändertVon');
		...
		var EditBtn = '<a href="#" onclick="window.open(\'https://portal.iabg.de/Lists/CampusApp_AbisZ/EditForm.aspx?ID='+DetailData.ID+'&Source=https%3A%2F%2Fportal.iabg.de%2FSitePages%2FCampusApp_AbisZ.aspx%3FItemId='+ID+'\', \'_self\'); return false;"><span class="spixDialogBtn">Eintrag editieren</span></a>';
		var MeldeBtn = '<a href="#" onclick="window.open(\'https://portal.iabg.de/bereich/services/Lists/SharePointMeldungen/NewForm.aspx?BezugTEXT='+$('#CampusAppTable_filter label input').val()+'&BezugTYPE=AbisZnew\', \'_self\'); return false;"><span class="spixDialogBtn">Fehler melden</span></a>';
		var PermaLinkBtn = '<a href="#" onclick="PermaLinkBtn(\''+ItemLink+'\'); return false;"><span class="spixDialogBtn">PermaLink</span></a>';
		var ShareBtn = '<a href="mailto:?subject=Informationen%20zum%20Thema%20'+encodeURIComponent(DetailData.Thema)+'&amp;body=%0D%0A%0D%0AHier%20gelangen%20Sie%20zum%20entsprechenden%20Eintrag:%20' + ItemLink + '"><span class="spixDialogBtn">Jemandem per Mail senden</span></a>';
		var CampusAppDialog = '<div style="width:800px;">';
		CampusAppDialog += '<h1  class="ms-core-pageTitle">'+DetailData.Thema+'</h1>';
		CampusAppDialog += '<table style="width: 100%;"><tr><td style="padding-right: 10px;"><span style="font-size: 12pt"><span style="font-size: 12pt;">'+DetailData.Kurzbeschreibung+'</span></span></td>';
		CampusAppDialog += '<td style="width: 150px; border-left: 2px solid #888888; padding-left: 10px; background-color: #EEEEEE;">';
		if(DetailData.ThemenbereichListe !== '') {
			CampusAppDialog += 'Themenbereich(e): <br /><span style="font-weight: bold;">'+DetailData.ThemenbereichListe+'</span><br /><br />';
		}
		var AuthorLync = spix_UserProfile_ShowLyncByMail(DetailData.GeändertVon.GeschäftlicheEMailAdresse,DetailData.ID);
		CampusAppDialog += '<span style="font-size:8pt; color: #888888;">Letzte Änderung:<br /> '+AuthorLync+DetailData.GeändertVon.Titel+' <br /> '+DetailData.GeändertDate+'';
		CampusAppDialog += '</td></tr></table>';
		CampusAppDialog += '<br /><div class="BtnLine">'+ShareBtn+PermaLinkBtn+MeldeBtn+EditBtn+'</div>';
		CampusAppDialog += '</div>';
...
function spix_CampusApp_ModView () {
	...
	var VorschlagBtn = '<input type="button" name="VoteNewItem" id="VoteNewItem" value="Thema  '+$('#CampusAppTable_filter label input').val()+'  vorschlagen" title="" onclick="window.open(\'https://portal.iabg.de/bereich/services/Lists/SharePointMeldungen/NewForm.aspx?BezugTEXT='+$('#CampusAppTable_filter label input').val()+'&BezugTYPE=AbisZnew\', \'_blank\'); return false;" style="background-color: #004993; color: #EEEEEE;" />';
	if($('.dataTables_empty').length > 0) {
		$('.dataTables_empty').html('Sie haben Ihr Thema nicht gefunden<br />'+VorschlagBtn+'<br />ODER Sie erstellen einen neuen Eintrag, wenn Sie die Antwort schon kennen.');
	}
...
function spix_CampusApp_PrintList() {
	if($('.ItemSelect:checked').length !== 0) {
		...
		$('.ItemSelect:checked').each(function() {
			if($(this).val() !== "All") {
				ListDataRawO[$(this).val()].Marker = ListDataRawO[$(this).val()].Thema.substr(0,1);
				DataO.push(ListDataRawO[$(this).val()]);
			}
		});
		var ActTimeStampe = new Date();
		//ActTimeStampe+'';$('#CampusAppInputFilterField').val(),$('#CampusAppTable_filter label input').val()
		var Signature = "Onlineausdruck erstellt am <b>"+ActTimeStampe.toLocaleString()+"</b> für <b>"+spix_UserProfile_GetCurrentUser('Title')+"</b><br />Daten gefiltert nach <b>'"+$('#CampusAppTable_filter label input').val()+"'</b> in <b>"+$('#CampusAppInputFilterField').val()+"</b>";
		spx_visux_PrintListByTemplate("DocTemplUrl:/CampusApp_Assets/AbisZ/AbisZ_Deckblatt.cfg;ItemTemplUrl:/CampusApp_Assets/AbisZ/AbisZ_Eintrag.cfg;Print_Signature:"+encodeURIComponent(Signature)+";Link:"+encodeURIComponent(window.location.href)+";Source:DataO",DataO);
		/*var HtmlResult = spix_visux_GetListByTemplate("DocTemplUrl:/CampusApp_Assets/AbisZ/Deckblatt.cfg;ItemTemplUrl:/CampusApp_Assets/AbisZ/Eintrag.cfg;Print_Signature:"+encodeURIComponent(Signature)+";Link:"+encodeURIComponent(window.location.href)+";Source:DataO",DataO);
		console.log(HtmlResult);
		var CssResult = null;
		$.ajax({url:'/CampusApp_Assets/AbisZ/Style.cfg', dataType: 'text', async: false, success: function(response) { CssResult = response; } });

		$('#CampusAppToolBar').after('<div id="TmpPdfForm" style="display: none;"><form id="Pdf" action="https://pdfe.iabg.de/serv.pdfe/CampusApp_AbisZ.php" target="_blank" method="post"><textarea name="ContentMain" cols="50" rows="10">'+HtmlResult+'</textarea><textarea name="Style" cols="50" rows="10">'+CssResult+'</textarea><input name="ApiKey" type="hidden" size="30" maxlength="30" value="CampusApp_AbisZ_PDFePrintList"></form></div>');
		console.log(HtmlResult);
		$('#Pdf').submit();
		$('#TmpPdfForm').remove();*/
	} else {
