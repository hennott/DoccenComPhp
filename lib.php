...
//@m:
$GLOBALS['AppVersion']['Lib'] = 'v0.1.7 (2016-05-13)';
...
function ab5_InitDoccen() {
	//@F:ab5_InitDoccen;
	//@V:$GLOBALS['SetCfg'] < set.config;
	//@c:2016-05-13:Bei einer neuen Installation wird eine leere set.config erstellt.;
	if(!is_file('set.config')) {
		file_put_contents('set.config', '{}');
	...
	//@R:Verfügbare Updates von Doccen anzeigen inkl. automatisches Update der Applikation;
	//@V:$GLOBALS['LangS'];
	...
	foreach ($GLOBALS['LangS'] as $LangCode) {
		...
		if(is_file($LangCode.'.lang')) {
			...
			//@c:2015-10-27:Anpasung des JSON Outputs auf "prettyfy".;
			if(is_file('my.'.$LangCode.'.lang')) {
				$GLOBALS['Locale'][$LangCode] = json_decode(json_encode(array_replace_recursive(json_decode(file_get_contents($LangCode.'.lang'),true), json_decode(file_get_contents('my.'.$LangCode.'.lang'),true)), JSON_PRETTY_PRINT),false);
			}
		} else {
			$GLOBALS['Locale'][$LangCode] = json_decode(file_get_contents('en.lang'));
			if(is_file('my.en.lang')) {
				$GLOBALS['Locale'][$LangCode] = json_decode(json_encode(array_replace_recursive(json_decode(file_get_contents('en.lang'),true), json_decode(file_get_contents('my.en.lang'),true)), JSON_PRETTY_PRINT),false);
			}
		}
	}
...
function ab5_CrawleDir($Abic) {
	...
	//@c:2015-11-11:Rekursive Anfragen wurden nicht bearbeitet, wenn der Filter leer übergeben wurde.;
	$ActDir = $Abic['Path'];
	...
	if (isset($Abic['Filter']) && $Abic['Filter'] !== '') {
		$Filter = $Abic['Filter'];
...
function ab5_MakeDirR($Path) {
	//@F:ab5_MakeDirR;
	//@D:Diese Funktion legt einen kompletten Ordnerpfad an.;
	//@I:Path;
	//@O:Dertig angelegt Ordnerstruktur;
	...
	for($i=0;$i<count($PathS)-1;$i++) {
		if($PathS[$i] !== '') {
			$CurrentPath .= $PathS[$i].DIRECTORY_SEPARATOR;
