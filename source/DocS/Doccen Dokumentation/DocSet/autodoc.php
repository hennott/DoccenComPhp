...
//@m:
$GLOBALS['AppVersion']['AutoDoc'] = 'v0.1.9 (2016-02-15)';
...
function VisualDoc ($FullIndex) {
	...
	if(count($FullIndex) > 0 && $FullIndex !== '') {
		foreach($FullIndex as $IndexDoc) {
			...
			if($CountFileInner > 0){
				$DocSetFileS[] = '<a href="#" onclick="HideByClass(\'MainContent\');ShowById(\'File_'.$GLOBALS['FileKey'].'\');return false" class="MenuLink">'.$IndexDoc['FileName'].'</a>';
			}
		...
		if($Pos !== false) {
			...
			switch($Case) {
				...
				if($GLOBALS['V'] == 'Expert') {
					...
					if(!isset($F)) { $F=''; }
					if(!isset($H)) { $H=''; }
					$cThis = array('Raw'=>$c,'Date'=>$cDate,'Memo'=>$cMemo,'FilePath'=>$DocIndex['FilePath'],'FileName'=>$DocIndex['FileName'],'Func'=>$F,'Head'=>$H);
					if(isset($F) && $F <> '') {
						$DocIndex[$F]['c'][] = $cThis;
