// @D:Diese Datei beinhaltet die Anzeige der KfzDatenbank
console.log('CampusApKfzDb.js v0.0.0');
...
function spix_CampusApp_Init(PreFilter,FilterCol,FilterVal) {
	...
	FilterCol = FilterCol || '';
	FilterVal = FilterVal || '';
    if($('#CampusApp').length == 0) { $('script[src*="CampusAppKfzDb.js"]').parent().append('<div id="CampusApp"></div>'); }
	$('#CampusAppCfg').hide();
...
function spix_CampusApp_InitList(PreFilter,FilterCol,FilterVal) {
        setTimeout(function() {
            ListData = spix_ListData_GetAllItems('/infothek/_vti_bin/listdata.svc/CampusApp_KfzDatenbank',PreFilter,'Marke','Kennzeichen,Marke,Ansprechpartner,Bemerkung');
            var i = 0;
            ListDataO = new Array();
...
function spix_CampusApp_ItemDataMod(Element) {
	if(Element.TelephoneNumber == null || Element.TelephoneNumber == '+49 ') { 
		Element.TlephoneNumber = ''; 
	} 
	if(Element.Mobile == null || Element.Mobile == '+49 ') { 
		Element.Mobile = ''; 
	} 
	
	if(Element.IpPhone == null || Element.IpPhone == '+49 ') { 
		Element.IpPhone = ''; 
	}
			
	if(Element.PhysicalDeliveryOfficeName == null) { 
	...
	if(Element.DistinguishedName.indexOf('OU=Besprechungsräume') > 0) {
		Element.Mail = '';
	}
	if(Element.FacsimileTelephoneNumber == null) { 
		Element.FacsimileTelephoneNumber = ''; 
	}
	if(Element.Pager == null) { 
		Element.Pager = ''; 
	}
...
function spix_CampusApp_Load(PreFilter,FilterCol,FilterVal) {
	...
	if(spix_GetUrlPara('DirectLink')) {
		$('a[data-listid="'+spix_GetUrlPara('DirectLink')+'"]').click();
	}
}
...
function spix_CampusApp_PrintList() {
	if($('.PersSelect:checked').length !== 0) {
		...
		$('.PersSelect:checked ').each(function() {
			if($(this).val() !== "All" && ListDataRawO[$(this).val()].Company == 'IABG mbH') {
				ListDataRawO[$(this).val()].Marker = ListDataRawO[$(this).val()].Kennzeichen.substr(0,1);
		...
		spx_visux_PrintListByTemplate("DocTemplUrl:/SiteAssets/CampusApp/TelBook_Deckblatt.html;ItemTemplUrl:/SiteAssets/CampusApp/TelBook_Eintrag.html;Print_Signature:"+encodeURIComponent(Signature)+";Link:"+encodeURIComponent(Link)+";Source:DataO",DataO);
	} else {
...
function spix_CampusApp_AppendMailList() {
	...
	if($('.PersSelect:checked').length !== 0) {
		$('.PersSelect:checked').each(function() { 
			if($(this).val() !== "All" && ListDataRawO[$(this).val()].Company == 'IABG mbH') {
				...
				if(ThisMail !== undefined && ThisMail !== 'mailto://') {
					Mail += ThisMail + ';';
				}
			}
		});
	...
	window.prompt("Kopieren Sie diese Mailadressen mit Strg+C in Ihr Mailprogramm!", Mail);
}

function spix_CampusApp_GetDirectlink() {
	window.prompt('Kopieren Sie den folgenden Link, um Ihre Recherche direkt aufzurufen.', 'https://'+window.location.hostname+window.location.pathname+'?FilterVal='+$('#CampusAppTable_filter label input').val()+'&FilterCol='+$('#CampusAppInputFilterField').val());
}

//############################################################################

function spix_CampusApp_Vcard(VCardContent) {
...
function spix_CampusApp_VcardLink(VCardContent) {
	...
	$('#CampusAppVcardLink').html('<a href="data:text/plain;charset=UTF-8,'+VCardContent+'" download="vCard.vcf"><button type="button">Download als VCF</button></a> oder QR-Code scannen ');
}

function spix_CampusApp_DetailView(Id,IdSource) {
	IdSource = IdSource || 'List'; // else 'Source'
	//console.log(Id);
	//console.log(ListData[Id]);
	//console.log(DetailData);
	if(Id !== 'close') {
		if(IdSource == 'List') {
			var ThisId = ListData[Id].ID;
		} else {
			var ThisId = Id;
		}	
		var DetailDataGet = spix_CampusApp_GetProfileCaByPara(ThisId,'ID');
		//var DetailDataRaw = spix_CampusApp_GetProfileCaByPara(ThisId,'ID');
		var DetailData = DetailDataGet;
		var DetailDataRaw = DetailDataGet;
		//var DetailDataRaw = spix_CampusApp_GetProfileCaByPara(ThisId,'ID');
		//console.log(DetailDataRaw);
		try	{
		  var Lync = spix_UserProfile_ShowLyncByMail(DetailData.Mail,ThisId);
		}
		catch(e) {}
		//var Lync = spix_UserProfile_ShowCompactImgPres(DetailData[0].Mail);
		var CampusAppDialog = '<div style="width:800px; padding: 20px;">';
		if(DetailData.Manager !== null) {
			var ManagerDetailData = spix_CampusApp_GetProfileCaByPara(DetailData.Manager,'Index');
			if(ManagerDetailData == undefined) {
				var ManagerLink = '..';
			} else {
				var ManagerLink = '<a href="#" onclick="spix_CampusApp_DetailView(\''+ManagerDetailData.ID+'\',\'Source\'); return false;">'+ManagerDetailData.Kennzeichen+'</a>';
			}
		} else {
		...
		var VCard = 'BEGIN:VCARD\r\nVERSION:2.1\r\nFN:'+DetailDataRaw.Kennzeichen+'\r\nN:'+DetailDataRaw.Kennzeichen+'\r\nTITLE:\r\nTEL;CELL:'+DetailDataRaw.Mobile+'\r\nTEL;WORK;VOICE:'+DetailDataRaw.TelephoneNumber+'\r\nTEL;WORK;VOICE:'+DetailDataRaw.IpPhone+'\r\nTEL;;\r\nFAX:'+DetailDataRaw.FacsimileTelephoneNumber+'\r\nEMAIL;WORK;INTERNET:'+DetailDataRaw.Mail+'\r\nURL:http://www.iabg.de\r\nADR;WORK:'+DetailDataRaw.PhysicalDeliveryOfficeName+'\r\nORG:'+DetailDataRaw.Company+' ('+DetailDataRaw.Department+')\r\nEND:VCARD';
		...
		var DirectLink = 'https://'+window.location.hostname+window.location.pathname+'?DirectLink=-'+DetailData.ID+'-';
